...
public class Utils {
	...
	static {
		StringTokenizer st = new StringTokenizer("gif		image/gif "
				+ "jpg		image/jpeg " + "jpeg		image/jpeg " + "png		image/png "
				+ "mp3		audio/mpeg " + "m3u		audio/mpeg-url "
				+ "mp4		video/mp4 " + "mkv		video/x-matroska "
				+ "avi 		video/x-msvideo " + "ogv		video/ogg "
				+ "mov		video/quicktime " + "ogm		video/ogg "
				+ "ogg		application/x-ogg ");
		while (st.hasMoreTokens())
			theMimeTypes.put(st.nextToken(), st.nextToken());
	}

	private static String[] str = { ".gif", ".jpg", ".jpeg", ".png" };
	...
	public static String getMimeType(String fileName) {
		...
		mime = theMimeTypes.get(fileName.substring(extensionStart + 1)
				.toLowerCase());
		return mime;
	...
	public static boolean isRoot(String filePath) {
		...
		try {
			return (new SmbFile("smb://" + filePath).getParent().toString()
					.equalsIgnoreCase("smb://"));
