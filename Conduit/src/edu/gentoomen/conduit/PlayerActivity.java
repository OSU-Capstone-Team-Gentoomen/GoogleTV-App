...
public class PlayerActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    myVideoView.setOnCompletionListener(new OnCompletionListener() {
	    	...
	    	@Override //Will listen for the end of the video
	    ...
	    myVideoView.setOnPreparedListener(new OnPreparedListener() {
			...
			public void onPrepared(MediaPlayer mp) {							
				...
				try {
					fileHash = FileHasher.getMediaFileHash(FileListFragment.server.getFile().getInputStream(), 
				...
				if (fileHash != null) {
					...
					Cursor curse = getContentResolver().query(ResumeContentProvider.CONTENT_URI, projection, 
															  ResumeContentProvider.COL_HASH + "= '" + fileHash + "'", 
															  null, "");
					if (curse.getCount() == 1) {
						updateEntry = true;
						if (curse.moveToNext()) {
							Log.d(TAG, "Need to resume at point: " + curse.getInt(TIME_COL));
...
//			BrowserActivity.getVideoProgressBar().cancel();
	...
	public void onBackPressed() {
		...
		if (myVideoView != null && myVideoView.hasFocus() && file != null && fileHash != null) {							
			try {
				if (updateEntry)
					updateResumeMediaEntry(myVideoView.getCurrentPosition());					
				else
					insertNewResumeMedia(file.getName(), FileHasher.getMediaFileHash(file.getInputStream(), file.length()), DeviceNavigator.getPath(), myVideoView.getCurrentPosition());
			} catch (SmbException e) {				
				e.printStackTrace();
			} catch (IOException e) {				
				e.printStackTrace();
	...
	private void updateResumeMediaEntry(int currentPosition) {
		...
		getContentResolver().update(ResumeContentProvider.CONTENT_URI, values, ResumeContentProvider.COL_HASH + "='" + fileHash + "'", null);
		
	}

	private void insertNewResumeMedia(String name, 
									  String hash, 									  
									  String path, int played) {
		...
		Log.d(TAG, "Inserting Time: " + played + ", Hash: " + hash + ", Path: " + path + ", Name: " + name);
	...
	private void removeResumeMedia(String hash) {		
		getContentResolver().delete(ResumeContentProvider.CONTENT_URI, ResumeContentProvider.COL_HASH + "='" + hash+ "'", null);		
	}
